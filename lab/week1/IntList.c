...
{	
	...
	// fail if the input list L is not sorted
	...
	// new data is the smallest in the list
	else if (n->data <= L->first->data) {
		n->next = L->first;
		L->first = n;
	} 
	// new data is the greatest in the list
	else if (n->data >= L->last->data) {
		L->last->next = n;
		L->last = n;
	} 
	// new data is somewhere in the middle
	else {
		// "current" track the position of the list
		struct IntListNode *curr = L->first;
		// "previous" record the previous node in the list
		struct IntListNode *prev = NULL;

		// move to the next node until find the appropriate position
	...
	// after the insertion increase the size
	L->size++;
}
